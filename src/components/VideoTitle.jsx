...
const VideoTitle = ({movieTitle, moviePlot, moreInfo}) => {
  ...
  return (
    <div className="w-screen h-screen absolute bg-gradient-to-r from-black top-0 left-0 md:pt-20 md:px-20 pt-26 p-7 z-10 cursor-pointer text-white">
      ...
      <div className="md:w-1/3 flex mt-2 md:mt-6 gap-3">
        ...
        <button className="w-1/4 md:w-1/2 p-1 md:py-2 text-xs md:text-xl rounded-lg font-semibold bg-slate-400/50 cursor-pointer" onClick={moreInfoHandler}>More Info</button>
      </div>
    </div>
  )
}
